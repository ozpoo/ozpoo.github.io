...
const computeHoughMatches = (options) => {
  ...
  const medianProjectedDim =
    projectedDims[
      Math.floor(projectedDims.length / 2) -
        (projectedDims.length % 2 == 0 ? 1 : 0) -
        1
    ]
  ...
  const binX = Math.floor(
    ((maxVoteIndex % numXYAngleBins) % numXYBins) % numXBins
  )
  const binY = Math.floor(
    (((maxVoteIndex - binX) % numXYAngleBins) % numXYBins) / numXBins
  )
  const binAngle = Math.floor(
    ((maxVoteIndex - binX - binY * numXBins) % numXYAngleBins) / numXYBins
  )
  const binScale = Math.floor(
    (maxVoteIndex - binX - binY * numXBins - binAngle * numXYBins) /
      numXYAngleBins
  )
