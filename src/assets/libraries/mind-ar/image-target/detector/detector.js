...
class Detector {
  ...
  detect(inputImageT) {
    ...
    if (this.debugMode) {
      debugExtra = {
        ...
        dogPyramidImages: dogPyramidImagesT.map((t) =>
          t ? t.arraySync() : null
        ),
  ...
  _computeFreakDescriptors(extremaFreaks) {
    ...
    return tf.tidy(() => {
      ...
      return tf
        .engine()
        .runKernel('ComputeFreakDescriptors', { extremaFreaks, positionT })
    })
  }
    ...
    if (!this.tensorCaches.orientationHistograms) {
      tf.tidy(() => {
        ...
        this.tensorCaches.orientationHistograms = {
          radialPropertiesT: tf.keep(
            tf.tensor(radialProperties, [radialProperties.length, 3])
          )
        }
      })
    }
  ...
  _computeLocalization(prunedExtremasList, dogPyramidImagesT) {
    ...
    return tf.tidy(() => {
      ...
      for (let i = 0; i < localizedExtremas.length; i++) {
        ...
        const dxy =
          0.25 * (pixel[0][0] + pixel[2][2] - pixel[0][2] - pixel[2][0])
